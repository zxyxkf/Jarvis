  ...
  <div class="overlay" @click="emit('close')">
    <div class="modal" @click.stop>
      ...
      <div class="avatar-section">
        <div class="profile-avatar">{{ auth.user?.name?.slice(0, 2).toUpperCase() }}</div>
        <div>
      ...
      <div class="form-fields">
        ...
        <label class="field-label" style="margin-top:12px">新密码（留空不修改）</label>
...
.modal{background:var(--color-bg-secondary);border:1px solid var(--color-border);border-radius:20px;padding:32px;width:400px;max-width:90vw;box-shadow:0 24px 64px rgba(0,0,0,.5)}
...
.profile-avatar{width:48px;height:48px;border-radius:50%;background:var(--color-accent-muted);color:var(--color-accent);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;flex-shrink:0}
